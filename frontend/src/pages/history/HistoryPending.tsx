...
export default function HistoryPending() {
    ...
    const { data: orderData } = useQuery(
        "fetchOrdersPendingForUser", orderApiClient.fetchOrdersPendingForUser
    )
    const { mutate: updatePendingToCanceled, isLoading: pendingToCanceled } = useMutation(
        ({ id }: { id: string }) => orderApiClient.updateStatusPendingToCanceled(id),
        {
            onSuccess: () => {
                queryClient.clear()
                showToast({ message: "Cancel Successful!", type: "SUCCESS" })
            },
            onError: (error: Error) => {
    ...
    const handleUpdatePendingToCanceled = (event: React.MouseEvent<HTMLButtonElement, MouseEvent>, id: string) => {
        ...
        const isConfirm = window.confirm("Do you want to cancel the order while it's still not in the 'Shipping' status?!")
        if (isConfirm) {
                ...
                <div className="flex flex-col gap-4">
                    {currentOrders.map((order: any) => (
                        ...
                        >
                            ...
                            <p className="text-sm text-gray-700 dark:text-gray-400">
                                Status: {order.status === "pending" ? "Pending Shipment" : "Pending"}
                            </p>
